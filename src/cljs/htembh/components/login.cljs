...
(defn encode-auth [email pass]
  (->> (str email ":" pass) (b64/encodeString) (str "Basic ")))

(defn login! [fields error]
  (let [{:keys [email pass]} @fields]
    (reset! error nil)
    (ajax/POST "/api/login"
               {:headers {"Authorization"
                          (encode-auth (string/trim email) pass)}
                :handler #(do
    ...
    (fn []
      [c/modal
       ...
       [:div
        ...
        [c/text-input "Email" :email "enter a user name" fields]
